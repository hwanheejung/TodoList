...
const curUserInfo = JSON.parse(localStorage.getItem('currentUser'));
...
if(curUserInfo !== null){
    ...
    const TODOS_KEY = `${curUserInfo.email}_todos`;
    ...
    function paintCheckTodo(){
        const parsedCheck = JSON.parse(localStorage.getItem(TODOS_KEY));
        const checkBoxes = document.querySelectorAll('.todo-checkbox');
        parsedCheck.forEach((pc, idx) => {
            if(pc.checked === true){
    ...
    function handleCheckTodo(){
        ...
        checkBoxes.forEach((box) => {
            const clickedTodo = todos.find(todo => todo.id === parseInt(box.parentElement.id));
            box.addEventListener('click', () => {
                if(box.checked === true){
                    clickedTodo.checked = true;
                }else{
